...
@Serializable
internal data class ApiChannel(val header: Header, override val contents: Contents<Content>) :
    ApiBrowse() {
    @Serializable
    data class Header(val c4TabbedHeaderRenderer: C4TabbedHeaderRenderer) {
        ...
        data class C4TabbedHeaderRenderer(
            ...
            data class Badge(
                ...
                private object TypeSerializer : JsonTransformingSerializer<String>(
                    String.serializer()
                ) {
                    override fun transformDeserialize(element: JsonElement): JsonElement =
                        element.jsonObject["icon"]!!.jsonObject["iconType"]!!
                }
            }

            @Serializable
            data class HeaderLinks(
                val channelHeaderLinksRenderer: ChannelHeaderLinksRenderer? = null
            ) {
                @Serializable
                data class ChannelHeaderLinksRenderer(
                    ...
                    data class Link(
                        ...
                        private object EndpointSerializer : JsonTransformingSerializer<String>(
                            String.serializer()
                        ) {
                            override fun transformDeserialize(element: JsonElement): JsonElement =
                                element.jsonObject["urlEndpoint"]!!.jsonObject["url"]!!
                        }
                    }
                }
            }
