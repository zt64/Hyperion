...
import dev.zt64.hyperion.api.network.dto.ApiWatchEndpoint
...
@Serializable
internal data class ApiPlaylist(val header: Header, override val contents: Contents<G>) : ApiBrowse() {
    @Serializable
    ...
    data class SectionContent(
        val playlistVideoListRenderer: SectionListRenderer<
            @Serializable(Renderer.Serializer::class)
            Renderer
            >
    ) {
        @Serializable
...
internal typealias ApiPlaylistContinuation = Continuation<
    @Serializable(ApiPlaylist.SectionContent.Renderer.Serializer::class)
    ApiPlaylist.SectionContent.Renderer
    >