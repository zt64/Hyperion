...
internal data class AccountsList(val contents: Contents) {
    ...
    data class Contents(val accountSectionListRenderer: AccountSectionListRenderer) {
        @Serializable
        data class AccountSectionListRenderer(val contents: List<Content>, val footers: List<Footer>) {
            @Serializable
            ...
            data class Footer(val accountChannelRenderer: Renderer) {
                @Serializable
                data class Renderer(val navigationEndpoint: ApiNavigationEndpoint, val title: ApiText)
            }
        }
    }
    ...
    data class AccountItem(
        ...
        val accountPhoto: dev.zt64.hyperion.api.network.dto.ApiImage,
